...
class PaymentGatewaySetup
{
    ...
    public static function renderFrontendFormContent($cash_on_delivery = false)
    {
        ...
        if (!empty(get_static_option('cash_on_delivery_gateway'))) {
            $output .= '<li data-gateway="cash_on_delivery" class="'. (get_static_option('site_default_payment_gateway') == "cash_on_delivery" ? 'selected' : "") .'" >
                            <div class="img-select">';
            $output .= render_image_markup_by_attachment_id(get_static_option('cash_on_delivery_preview_logo'), null, null, __('Thanh toán khi giao hàng'));
            $output .= '</div></li>';
        }
